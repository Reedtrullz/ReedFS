...
- `RfsPFD.tsx` renders readable speed/altitude tapes, attitude/heading, low-altitude radio-altitude awareness, and an FMA row from the same truth modes the servo laws use; `N1` is shown from `apState.truth.thrustActive`, not from a cosmetic flag. PFD/debug telemetry selectors subscribe to primitive/derived values instead of the full `aircraft` object snapshot.
