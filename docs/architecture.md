...
- Pilot inputs, AP commands, AP controller state, and effective controls are separate store fields; AP can own elevator/aileron/throttle without mutating pilot-authored inputs, and AP PID/rate-limit integrators are explicit serializable state instead of module globals.
...
`src/store/simStore.ts` is the public compatibility API for Zustand consumers and owns the fixed-step `tick(timestamp)` heartbeat order. Domain-owned actions are assembled from `src/store/slices/`: `aircraftSlice.ts` for lifecycle/reset/scenario state, `inputSlice.ts` for pilot input sanitization and input-manager actions, `autoflightSlice.ts` for AP state ownership helpers, `routeSlice.ts` for route/wind updates, and `persistenceSlice.ts` for saved-scenario restore/save. The slice boundary must not change public action names (`startTakeoffRoll`, `setInput`, `setApState`, `setFlightPlan`, `reset`, `tick`) or reorder the central heartbeat.
...
- `flightPlanLoader.ts` keeps the KSEA→KPDX and ENVA→ENGM sample routes as canned sources/shapes. KSEA→KPDX terminates at synthetic KPDX 10R IF/FAF/RWY waypoints; ENVA→ENGM now terminates at synthetic ENGM 19R IF/FAF/RWY waypoints for the visible ENVA→ENGM proof. `fms/routeAdapter.ts` defines the RFMS-compatible route-edit seam around the shared `FlightPlan` shape: route sources, staged draft operations, DIRECT_TO, explicit DISCONTINUITY insertion, undo, and EXEC are pure data operations with unit coverage. This is not a CDU implementation yet; the visible browser route panel remains read-only/canned and route-status/LNAV ownership remains in the store until a dedicated UI wires those adapter operations into route recomputation.
...
- `GuidanceState` combines scenario phase, tutorial, checklist, coach messages, and alerts for the player-facing flow. It derives preflight/takeoff-roll/rotation/rejected-takeoff/positive-rate/climb plus approach/landing-rollout/landed phases and uses those phases to auto-select checklist and tutorial state while preserving explicit tutorial-step overrides. `ScenarioPanel` presents this in a named `Scenario and tutorial` region, with coach text exposed as a polite live `Coach status`; route status and AP truth remain adjacent store-owned displays.
- `scenarioPersistence.ts` saves cloneable scenario snapshots to `localStorage`, including AP controller integrator/rate-limit state for repeatable AP/A/T resumes, and `ScenarioPanel` exposes SAVE/LOAD controls with visible ignored/corrupt-save feedback. Running saves restore as paused so training loops do not surprise-resume.
- `controlBindings.ts`, `ControlsHelp`, and collapsed-by-default `ControlsSettings` provide a validated, visible keyboard/gamepad binding model for repeated play without covering primary instruments unless expanded; `Space` remains symmetric brakes while `Z`/`X` are momentary left/right brake controls that clear on release, blur, visibility change, and cleanup. `GamepadManager.ts` maps standard buttons to start/pause/reset, camera, overlay, audio, and backed MCP command intents in tests without requiring hardware; durable gamepad calibration UI/persistence remains deferred. Manual AP disconnect/override clears stale Boeing thrust flags for both `speedMode` and `n1`, and unbacked raw AP states are not treated as active control owners.

Known guidance follow-up:

- Add RFMS-backed route modification UI that exposes the existing adapter seam through visible controls, store updates, route-status recompute, and honest unavailable/EXEC feedback.
...
- Proof classes are intentionally separate. Unit/static gates validate source contracts; seeded/scoped browser helpers may set up exact aircraft/route/autoflight states to guard one behavior; manifest-listed black-box specs must use visible controls and visible readbacks only. Seeded/scoped proofs remain regression evidence, not full-flight/full-route, CI, deploy, or live evidence unless those layers are separately run and verified.
...
- `e2e/rfs-route.spec.ts` proves the KSEA sample route loads in-browser, exposes backed LNAV, decreases DTG while the route leg is flown, sequences the first and second KSEA route legs while backed LNAV/FMA truth remains active, runs a single-store multi-gate route progression proof through the OLM and BTG gates, starts on the KPDX 10R final approach leg to configure an airborne approach with CMD A + LNAV + SPEED still backed while vertical FMA remains OFF, proves a scoped configured-approach-to-manual-handoff state where AP/FMA/thrust truth is OFF, AP command ownership is empty, and pilot/effective axes match while the KPDX 10R threshold leg remains loaded, separately proves reset cleanup after that manual handoff clears the route, AP state, FMA/AP modes, AP commands, running status, and guidance back to preflight without mixing reset state into active-route samples, bridges the KSEA final route configured approach/manual AP-FMA-thrust OFF handoff into a same-store KPDX 10R short-final manual landing, braking rollout, and reset cleanup proof whose KPDX short-final landing snapshots recompute near-destination route status instead of carrying stale pre-seed distance, and now adds an extended pre-handoff final-route descent segment that stays on the KPDX 10R threshold leg with CMD A + LNAV + SPEED backed, vertical FMA OFF, airborne approach guidance, and at least 1.0 NM / 300 ft additional closure before the truthful manual handoff into that KPDX 10R landing bridge. These are clean-climb, scoped short-final approach-to-touchdown/rollout/reset, seeded descent-to-approach/landing bridge, scoped route-progression, scoped final-leg configured-approach, scoped configured-approach-to-manual-handoff-and-reset, KSEA-to-KPDX 10R landing bridge with recomputed KPDX short-final route status, and extended pre-handoff route-coupled descent bridge proofs, not full-flight, full-route, continuous route-coupled KPDX landing, continuous route-coupled descent/approach/landing, VNAV, CI, deploy, or live claims.

- `e2e/rfs-full-flight-blackbox.spec.ts` is the visible-control ENVA→ENGM full-flight acceptance gate for this slice: it opens the app, loads the default ENVA→ENGM canned route, performs visible ENVA takeoff/positive-rate/gear/flap cleanup, proves backed LNAV/SPEED/VS route-climb smoke on the ENVA→ENGM route, resets through the visible control, selects the ENGM 19R short-final scenario, and proves ENGM touchdown/rollout/STOPPED/reset readbacks. It is a complete local browser proof for the requested ENVA→ENGM flow, but it still uses a deliberate short-final reset boundary for the destination landing; it is not continuous route-coupled descent/approach/landing, official ENGM procedure data, certified 737 realism, CI, deploy, or live evidence.
...
2. Worker physics: codec and worker entry scaffolding exist, `src/sim/simulationRuntime.ts` provides main-thread, worker-handler parity, and experimental browser-Worker runtime adapters. `VITE_RFS_WORKER_PHYSICS=1` now creates a real module Worker behind the default-off flag, with request/response IDs, one-request backpressure, timeout/error main-thread fallback, and `dispose()`. The current store tick remains synchronous, so browser-worker selection exposes Worker-backed `stepAsync()` while sync `step()` still falls back to main-thread physics until the frame scheduler becomes async-aware; no default-on migration and no SharedArrayBuffer/COOP/COEP dependency is introduced. A production worker migration requires a separate async scheduler/store bridge plan covering input, route, AP-controller, wind, lifecycle errors, pause/resume/reset, and browser parity before any docs may describe worker physics as active.
