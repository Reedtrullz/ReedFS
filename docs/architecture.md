...
- `ground.ts` distributes normal force across gear stations, computes rolling friction/brake forces and normal-force-scaled tire side forces from loaded stations, fades nosewheel steering out as speed rises, prevents stationary steering from creating motion, and records touchdown sink rate.
...
1. Advanced gear/tire model details: normal-force-scaled tire side-load/cornering stiffness is now in the ground model; remaining gaps include anti-skid, asymmetric braking, dynamic oleo spring/damper response, broader crosswind scenario coverage, and non-runway surface support beyond the current station/load/friction/side-load/steering/touchdown model.
