...
- `vnav.ts` looks ahead over the remaining route, resolves future altitude/speed constraints, reports target waypoint/distance/TOD metadata, and exposes an explicit lifecycle (`ARMED`, `PATH`, `ALT_CAPTURE`, `ALT_HOLD`, `COMPLETE`). Armed pre-TOD VNAV stays truthfully shown as VNAV without AP/PFD pitch commands; PATH/ALT* provide conservative vertical-speed targets, and managed ALT_HOLD preserves the VNAV constraint altitude instead of reverting to the MCP window.
