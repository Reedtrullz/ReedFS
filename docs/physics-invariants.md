...
- `sampleSupportedAirportSurface()` classifies supported ENVA/ENGM/KSEA/KPDX prepared runway rectangles as prepared runway and off-rectangle supported-airport ground as `offRunway` with separate rolling/brake/side friction scales. Current ENGM coverage is the synthetic 19R/01L training runway fixture for ENVA→ENGM proof; current KPDX coverage is the 10L/28R, 10R/28L, and 03/21 runway rectangles.
...
- `src/viewport/__tests__/runwayData.test.ts` and `src/sim/__tests__/runwaySurface.test.ts` cover the ENVA/ENGM/KSEA/KPDX runway catalog, generic sampler, KSEA wrapper compatibility, ENGM 19R synthetic runway/approach fixture, and KPDX off-runway fallback behavior.
