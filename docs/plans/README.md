...
- `2026-06-17-rfs-enva-engm-full-flight-remediation.md` — current ENVA→ENGM issue log, implementation plan, and proof-boundary ledger: adds synthetic ENGM 19R runway/approach surface support, converts the ENVA route to ENGM 19R IF/FAF/RWY waypoints, adds the ENGM 19R short-final landing scenario, and converts the slow visible-control full-flight gate to ENVA→ENGM route/climb plus ENGM landing rollout/reset.
- `../reviews/2026-06-16-rfs-e2e-proof-stability-closeout.md` — closeout for the June 16 proof-stability slice: local default E2E, aggregate check, visual gate, and separate explicit full-flight gate are green after migrating post-positive-rate black-box assertions to durable EngineStrip readbacks and keeping mouse cleanup controls visible only through dirty climb; still no CI/live/continuous-full-route/source-backed realism claim.
