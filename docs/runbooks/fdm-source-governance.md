...
- Source ID: `ourairports-norway-runway-endpoints-2026-06-17`.
- Citation: OurAirports `airports.csv` and `runways.csv` from immutable mirror commit `fe021a6f396a72f73cbaa160a710ef96f7fb21bf` (2026-06-17). The generated module records the exact raw URLs and SHA-256 hashes for both inputs.
- License/redistribution: public-domain / Unlicense source data; OurAirports states the CSV data is public domain and comes with no accuracy or fitness guarantee.
- Data group: `runway/airport`.
- Confidence: `derived-from-source`.
- Public claim boundary: "RFS uses redistributable OurAirports runway endpoint geometry for simulator runway placement; not official AIP/procedure data, not certified for navigation, and not suitable for real-world use."
- Versioned data files: `src/viewport/norwayRunwayData.generated.ts`, generated by `scripts/generate-norway-runways.mjs`.
- Runtime consumers: `src/viewport/runwayData.ts`, `src/viewport/RunwayLayer.tsx`, `src/sim/runwaySurface.ts`, and downstream physics ground-contact sampling.
- Verification tests: `src/viewport/__tests__/runwayData.test.ts`, `src/sim/__tests__/runwaySurface.test.ts`, and `src/viewport/__tests__/RunwayLayer.test.tsx`.
- Derivation notes / limitations: includes Norway/Svalbard `small_airport`, `medium_airport`, and `large_airport` rows with open non-water/non-ice runways, positive dimensions, and complete non-zero endpoint coordinates. Excludes heliports, seaplane bases, closed airports, closed runway rows, water/ice surfaces, rows without complete endpoints, and 0/0 endpoint placeholders. Existing hand-authored ENVA 09 and synthetic ENGM 19R autoland fixtures remain in the runtime catalog to preserve current scenario/autoland behavior.
