...
As of the current repository state, the release-hardening/playability sequence through Task 10.4 and the June Tasks 2-11 guidance-truth slice are implemented. AP and FMA now share route-status-derived navigation truth, guidance/checklists/tutorials are phase-aware, the ENVA tutorial has a deterministic Playwright takeoff-to-clean-climb proof, and the KSEA sample has first/second route-leg sequencing browser proofs. The remaining roadmap is now advanced realism, full-route/full-flight proof beyond clean climb and the route-leg sequencing proofs, visual/layout polish, data quality, and deploy verification.
...
- KSEA sample route loads in-browser, exposes backed LNAV, keeps FMA LNAV, decreases DTG, and sequences the first and second route legs during scoped browser proofs.
...
- Full-route/full-flight proof beyond the current deterministic ENVA clean-climb and KSEA route-leg sequencing browser tests, including approach/landing coverage and broader FMS behavior.
...
The immediate follow-ups from the 2026-05-26 dogfood are completed historical records. The June next slice now focuses on AP/FMA truth plus full-flight proof beyond the already-implemented ENVA clean-climb and KSEA route-leg sequencing browser proofs.
