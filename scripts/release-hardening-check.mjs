...
check(dockerfile.includes("RUN apk upgrade --no-cache libcrypto3 libssl3 libxml2 libexpat c-ares curl libcurl libuuid"), "Dockerfile must refresh fixed Alpine TLS/XML/network/UUID packages after the pinned nginx base image");
...
check(playbook.includes("127.0.0.1::{{ container_port }}"), "Ansible canary must request a dynamically allocated loopback port");
check(playbook.includes("canary_info.container.NetworkSettings.Ports") && playbook.includes("canary_port_bindings[0].HostPort"), "Ansible must inspect and use the dynamically allocated canary port");
check(!playbook.includes("3004"), "Ansible must not reserve fixed host port 3004 for the RFS canary");
check(countMatches(playbook, /127\.0\.0\.1:\{\{ host_port \}\}:\{\{ container_port \}\}/g).length >= 2, "Ansible production and rollback must bind the public loopback host port to container port 8080");
