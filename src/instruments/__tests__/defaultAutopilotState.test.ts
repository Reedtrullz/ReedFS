...
describe('createDefaultAutopilotState', () => {
  ...
  it('seeds Boeing selected targets from the current aircraft state without backing AP command channels', () => {
    ...
    const expectedIas = Math.round(Math.max(0, computeDerived(aircraft).ias));
    ...
    expect(ap.boeing.speed).toBe(expectedIas);
    expect(ap.boeing.speed).toBeCloseTo(149, 0);
    expect(ap.truth.autopilotStatus).toBe('OFF');
