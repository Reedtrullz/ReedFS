...
describe('useSimStore', () => {
  ...
  it('setApState stores autopilot state', () => { useSimStore.getState().setApState({ truth: { autopilotStatus: 'CMD_A' } } as any); expect(useSimStore.getState().apState).toBeTruthy(); });
  it('reset clears apState', () => { useSimStore.getState().setApState({} as any); useSimStore.getState().reset(); expect(useSimStore.getState().apState).toBeNull(); });
});
