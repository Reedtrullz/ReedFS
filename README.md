...
- Ground contact now has explicit ground state, per-gear station loads, dynamic oleo spring/damper compression, runway-normal constraints, tire braking/rolling friction, anti-skid brake limiting, symmetric and side-specific brake commands for player differential braking, asymmetric brake-force helpers, normal-force-scaled tire side-loads, rudder-pedal-limited nosewheel steering, low-speed taxi, rollout braking, crosswind/weathercocking, and crosswind approach/touchdown/rollout scenario regressions, gear-up runway-tangent belly/crash slide damping, touchdown damping, ground effect, normal-force liftoff gating, and phase handling. It samples dry gameplay runway rectangles derived from an immutable OurAirports Norway/Svalbard endpoint snapshot plus the existing KSEA/KPDX fixtures, distinguishes prepared-runway contact from off-runway ground contact, and treats `GroundState.onRunway` as prepared-runway surface status rather than generic ground contact; off-runway gear/belly/crashed contact stays explicit, with higher rolling resistance and reduced brake/side grip while preserving ground-relative velocity and runway-normal constraints. Off-runway elevation fallback is still a simplified nearest-supported-runway reference, not terrain mesh collision.
- Ground realism non-claims: OurAirports supplies only community runway endpoint geometry. The simulator turns those endpoints into simplified dry gameplay rectangles; it does not provide official procedure/navigation data, displaced-threshold or slope fidelity, wet/contaminated surfaces, airport aprons/taxiway networks, source-backed tire relaxation/heat/wear or high-fidelity side-load curves, broad terrain mesh collision, or certified crosswind/rollout performance behavior.
...
- FDM/performance source-data governance is documented in [`docs/runbooks/fdm-source-governance.md`](docs/runbooks/fdm-source-governance.md). Current FDM/performance data remains gameplay-placeholder unless versioned metadata says otherwise; only the Norway/Svalbard runway endpoint geometry has a reviewed immutable source packet, while aero, engine, gear, tire/brake, performance-card, procedure, and broader airport-surface work remains blocked or synthetic.
- CI enforces lint, typecheck, tests, and production build before publish/deploy.

The next major enhancements are the prioritized realism/product phases documented in `docs/roadmap.md`, including source-backed ground-handling/tire tuning, broader terrain mesh collision and airport-surface fidelity beyond simplified runway rectangles, RFMS route-edit/FMA lifecycle fidelity, worker timing, flight-model data quality, and product polish.
...
- Weather/atmosphere: partial. RFS parses METAR wind/QNH/temperature data, applies QNH and temperature to density-altitude physics, and derives baro-indicated PFD altitude from live QNH; visibility rendering tied to METAR layers, precipitation, and weather-driven scene degradation remain deferred.
...
- Scene loading/error states: partial. RFS has an app ErrorBoundary, a visible `SCENERY DEGRADED` status for missing Cesium Ion scenery, degraded ellipsoid fallback, `SCENERY ERROR` surfacing for runtime OSM-buildings and render failures, and a manual `RETRY SCENERY` remount; richer automatic recovery and broader network-failure UX remain deferred.
...
Physics runs on a default-on browser `Worker`. The bridge slice provides runtime adapters, main-thread/worker-handler parity tests, and the browser module Worker selected by default; the async `FrameScheduler`/`useSimLoop`/`simStore` bridge awaits `stepAsync()` while preserving input, route, AP, wind, error-fallback, pause/resume/reset, and visual/E2E parity. `VITE_RFS_WORKER_PHYSICS=0` is the documented escape hatch that forces the main-thread adapter.
...
`VITE_RFS_WORKER_PHYSICS` is parsed by `src/config/workerPhysics.ts` as the feature flag for browser-Worker physics wiring and is default-on. Truthy tokens are `1`, `true`, `yes`, `on`, and `enabled`; false/off tokens are `0`, `false`, `no`, `off`, `disabled`, and an empty value, which force main-thread physics as an escape hatch. Invalid values fall back safely to main-thread physics with an explanatory config reason.

By default, `src/sim/simulationRuntime.ts` instantiates a real browser module Worker and exposes Worker-backed `stepAsync()` with request/response IDs, one-request backpressure, timeout/error main-thread fallback, and `dispose()`. `simStore.tickAsync()` bridges the frame loop to the worker runtime when it exposes `stepAsync()`, coalescing frames while a batch is in flight and preserving pilot inputs changed mid-flight; sync `step()` still falls back to main-thread physics when the async path cannot be used. The runtime does **not** require SharedArrayBuffer/COOP/COEP, and RFS still does **not** set COOP/COEP headers.
